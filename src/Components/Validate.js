class Validate {
	static isValid(config){
		if(!config.touched && !config.value.length) return { status : true }
		switch(config.type){
			case 'text':
				if(config.condition.minLength && config.value.length < config.condition.minLength){
					return { status : false, msg : `Content should be of minimum ${config.condition.minLength} characters`};
				}
				if( config.condition.maxLength && config.value.length > config.condition.maxLength){
					return { status : false, msg : `Content should be of maximum ${config.condition.maxLength} characters`};
				}
				return { status : true };
			case 'number':
				if(isNaN(config.value)){
					return { status : false, msg : 'Its not a number'}
				}
				if(config.condition.minLength && config.value.length < config.condition.minLength){
					return { status : false, msg : `Number should be of minimum ${config.condition.minLength} digits`};
				}
				if( config.condition.maxLength && config.value.length > config.condition.maxLength){
			...
			case 'date' :
				...
				return { status : true };
			default :
				return { status : false, msg : 'Invalid Content'};
		}
	}
}
